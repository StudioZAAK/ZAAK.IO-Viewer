...
function setManagerMode(_id){
  managerID = _id;

  if(manager !== null)
    manager.setMode_(_id);
}
...
function setProject ( json ){
  ...
  // scene = objLoader.parse( json.scene );

  //LoadingManager
  // var geometry = new THREE.SphereGeometry( 0.2, 32, 32 );
  // var material = new THREE.MeshBasicMaterial( {color: 0x000000, transparent:true, opacity:1, side: THREE.BackSide} );
  // loader = new THREE.Mesh( geometry, material );
...
function onDocumentTouchEnd( event ) {
  ...
  if(eventObject !== null && rayEnd[eventObject.uuid] && manager.getViewer().id == "CardboardV2") {
    dispatch( rayEnd[ eventObject.uuid ] );
    eventObject = null;
...
function crossHairScaling(_dir){
  if(manager.isVRMode()){
    if(_dir){
      // TweenMax.to(".Absolute-Center-Loader", maxLookTime, {className:"+=Absolute-Center-Big"});
...
function loadSkyImages(){
  ...
  //LoadTextures
  for(var ii = 0; ii < skyNames.length; ii++){
    for(var i = 0; i < 6; i++){

      // imageLoader.load( 'img/skyboxes/'+ skyNames[ii] +'/pano_'+ i +'.jpg' );    
    }
  }
}

function recreateSky(_folderName, _position){

  var urlsNew = [
    'img/sky/'+folderName+'/pano_2.jpg',
    'img/sky/'+folderName+'/pano_0.jpg',
    'img/sky/'+folderName+'/pano_4.jpg',
    'img/sky/'+folderName+'/pano_5.jpg',
    'img/sky/'+folderName+'/pano_1.jpg',
    'img/sky/'+folderName+'/pano_3.jpg',

  ];

  var shader = THREE.ShaderLib['cube']; // init cube shader from built-in lib
  shader.uniforms['tCube'].value = THREE.ImageUtils.loadTextureCube(urlsNew); // apply textures to shader
